...
class CompleteHabitView(APIView):
    permission_clases = [IsAuthenticated]   # El usuario debe estar autenticado
    ...
    def post(self, request, habit_id):
        try:
            ...
            return Response({'error': 'Hábito no encontrado.'}, status=status.HTTP_404_NOT_FOUND)   # Si no se encuentra el hábito, lanza un error.
        ...
        # Crear el registro como completado
        ...
        serializer = HabitRecordSerializer(record)  # Guarda el registro del hábito en el serializer
