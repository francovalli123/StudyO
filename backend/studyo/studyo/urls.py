...
from apps.reading.views import BookListCreateView, BookDetailView, BookProgressUpdateView, BookFileView
...
urlpatterns = [
    ...
    path('api/user/me/avatar/', CurrentUserAvatarView.as_view(), name='current_user_avatar'),
    path('api/user/me/photo/', CurrentUserAvatarView.as_view(), name='current_user_photo'),
    ...
    path('api/books/<int:pk>/file/', BookFileView.as_view(), name='book_file'),
...
if settings.DEBUG:
    ...
    # Render/Whitenoise do not serve MEDIA files by default.
    # Keep a minimal media route so uploaded avatars are reachable in production.
    urlpatterns += [
