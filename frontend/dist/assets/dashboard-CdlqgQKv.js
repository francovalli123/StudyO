import"./modulepreload-polyfill-B5Qt9EMX.js";import{a as Ke,i as Ze,g as q,b as L,c as Je,d as Le,e as Ye,f as Ve,h as Oe,j as Qe,B as Xe}from"./auth-gate-B8HnkSQA.js";import{i as et,s as tt,a as ge}from"./confirmModal-L4nKbxRN.js";import{g as F,t as S,s as ot,a as ae}from"./responsive-DwadFwRg.js";import{h as nt,s as at,g as ue,a as R,b as W,c as Ie,d as rt,p as it}from"./onboarding-B5C-bpxo.js";document.addEventListener("DOMContentLoaded",function(){const e=t=>{if(!t)return;const a=document.getElementById("firstName");if(a){const i=t.first_name?t.first_name:t.username;a.textContent=i}const s=document.getElementById("dashAvatarImg"),c=t.avatar||t.avatar_url||t.photo||t.profile_image;s instanceof HTMLImageElement&&c&&(s.src=c)},o=window.__studyoAuthState;o&&o.status==="authenticated"&&o.user&&e(o.user),window.addEventListener("auth:state",t=>{const a=t&&t.detail?t.detail:null;a&&(a.status==="authenticated"&&e(a.user),a.status==="unauthenticated"&&(window.location.href="/login"))});const r=document.getElementById("profileAvatarBtn");r&&r.addEventListener("click",()=>{window.location.href="/profile"});const n=document.getElementById("logoutBtn");n&&n.addEventListener("click",async t=>{t.preventDefault();try{await Ke(),window.location.href="/"}catch(a){console.error("Error al cerrar sesión:",a),window.location.href="/"}})});Ze({loginPath:"/login"});let me=!1;const st=typeof window<"u"&&(window.matchMedia?.("(hover: none), (pointer: coarse)").matches||navigator.maxTouchPoints>0);function D(){typeof lucide>"u"||me||(me=!0,window.requestAnimationFrame(()=>{me=!1,lucide.createIcons()}))}function ct(e){const o=window;if(typeof o.requestIdleCallback=="function"){o.requestIdleCallback(e,{timeout:1200});return}window.setTimeout(e,0)}function Be(){function e(){document.body.classList.add("concentration-mode");const t=document.getElementById("pomodoroSettingsModal");t&&(t.style.display="none");try{const a=document.documentElement;a.requestFullscreen&&a.requestFullscreen().catch(()=>{})}catch{}}function o(){document.body.classList.remove("concentration-mode");try{document.fullscreenElement&&document.exitFullscreen()}catch{}try{window.hideEnterToast?.()}catch{}const t=document.getElementById("pomodoroSettingsModal");t&&(t.style.display="none"),setTimeout(()=>{try{window.showExitToast?.()}catch{}},120)}const r=document.getElementById("concentrationToggleBtn"),n=document.getElementById("concentrationExitBtn");r&&r.addEventListener("click",t=>{if(t.preventDefault(),document.body.classList.contains("concentration-mode"))o();else{e();try{window.showEnterToast?.()}catch{}}}),n&&n.addEventListener("click",t=>{t.preventDefault(),o()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&document.body.classList.contains("concentration-mode")&&o()}),document.addEventListener("fullscreenchange",()=>{!document.fullscreenElement&&document.body.classList.contains("concentration-mode")&&o()})}function lt(){const e=S(),o=document.getElementById("concentrationEnterToast");if(o){const n=o.querySelector(".title");n&&(n.textContent=e.dashboard.concentrationEnterTitle);const t=n?.nextElementSibling;t&&(t.textContent=e.dashboard.concentrationEnterBody);const a=o.querySelector(".close-x");a&&a.setAttribute("title",e.common.close)}const r=document.getElementById("concentrationExitToast");if(r){const n=r.querySelector(".title");n&&(n.textContent=e.dashboard.concentrationExitTitle);const t=n?.nextElementSibling;t&&(t.textContent=e.dashboard.concentrationExitBody);const a=r.querySelector(".close-x");a&&a.setAttribute("title",e.common.close)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Be):Be();function dt(e){if(!e)return"No programado";const o=new Date(e),r=new Date,n=o.getTime()-r.getTime(),t=Math.ceil(n/(1e3*60*60*24));return t<0?"Pasado":t===0?"Hoy":t===1?"Mañana":t<=7?`En ${t} días`:o.toLocaleDateString("es-AR",{day:"numeric",month:"short"})}function je(e){return e&&{1:"Alta",2:"Media",3:"Baja"}[e]||"Sin prioridad"}function $e(e){return e&&{1:"#ef4444",2:"#f59e0b",3:"#10b981"}[e]||"#71717a"}async function ut(){try{const e=await L("/habits/"),o=e.length,r=e.length>0?Math.max(...e.map(i=>i.streak)):0,n=e.length>0?Math.round(e.reduce((i,d)=>i+d.streak,0)/e.length):0,t=document.getElementById("total-habits"),a=document.getElementById("max-streak"),s=document.getElementById("avg-streak");t&&(t.textContent=o.toString()),a&&(a.textContent=r.toString()),s&&(s.textContent=n.toString());const c=document.getElementById("recent-habits");if(c){const i=[...e].sort((d,m)=>m.streak-d.streak).slice(0,5);i.length===0?c.innerHTML='<p class="empty-state">No tenés hábitos aún. <a href="/habits">Creá tu primer hábito</a></p>':c.innerHTML=i.map(d=>`
  ...
  `}async function J(){try{const e=await L("/pomodoro/"),o=(await q()).timezone||Intl.DateTimeFormat().resolvedOptions().timeZone,r=(f,p)=>new Intl.DateTimeFormat("en-CA",{timeZone:p,year:"numeric",month:"2-digit",day:"2-digit"}).format(f),n=r(new Date,o),t=e.filter(f=>{const p=new Date(f.start_time);return r(p,o)===n}),a=t.reduce((f,p)=>f+p.duration,0),s=t.length,c=Y(o),i=e.filter(f=>new Date(f.start_time)>=c).length,d=document.getElementById("sessions-today"),m=document.getElementById("minutes-today");d&&(d.textContent=s.toString()),m&&(m.textContent=a.toString());try{document.dispatchEvent(new CustomEvent("pomodoro:sessionsUpdated",{detail:{totalSessionsThisWeek:i,totalSessionsToday:s}}))}catch{}const y=document.getElementById("recent-sessions");if(y){const f=e.sort((p,b)=>new Date(b.start_time).getTime()-new Date(p.start_time).getTime()).slice(0,5);f.length===0?y.innerHTML='<p class="empty-state">No tenés sesiones de Pomodoro aún</p>':y.innerHTML=f.map(p=>{const h=new Date(p.start_time).toLocaleDateString("es-AR",{day:"numeric",month:"short"});return`
                ...
                `,D();return}const b=y,x=450/6;let w="";const E=[];if(i.forEach((I,g)=>{const k=g*x,T=m-I/d*m;E.push({x:k,y:T})}),E.length>1){let I=`M ${E[0].x} ${E[0].y}`;for(let g=1;g<E.length;g++){const k=E[g-1],T=E[g],A=k.x+(T.x-k.x)/2,U=k.y,P=T.x-(T.x-k.x)/2,$=T.y;I+=` C ${A} ${U}, ${P} ${$}, ${T.x} ${T.y}`}w=I}else E.length===1&&(w=`M ${E[0].x} ${E[0].y}`);const O=r.dashboard.weekDays;b.innerHTML=`
            ...
            `,st||b.querySelectorAll("circle").forEach((g,k)=>{const T=i[k],A=(T/60).toFixed(1),U=O[k];g.addEventListener("mouseenter",P=>{const $=document.createElement("div");$.id="rhythm-tooltip",$.className="fixed bg-dark-card border border-purple-500/30 rounded-lg px-3 py-2 text-sm text-white shadow-lg z-50 pointer-events-none",$.innerHTML=`
                ...
                `,D();return}p.style.background=`conic-gradient(${f})`;const h=document.getElementById("focus-total-hours");if(h)if(i<60)h.textContent=`${i}m`;else{const w=Math.floor(i/60),E=i%60;h.textContent=E===0?`${w}h`:`${w}h ${E}m`}const x=document.getElementById("focus-legend");x&&(x.innerHTML=d.slice(0,4).map((w,E)=>`
            ...
            `,D();return}const a=t[0],s=a.start_time.substring(0,5),c=a.end_time.substring(0,5),i=typeof F=="function"?F():"es",d=new Date(`${a.date}T00:00:00`),m=new Date;m.setHours(0,0,0,0);let y;if(d.getTime()===m.getTime())y={es:"Hoy",en:"Today",pt:"Hoje",zh:"今天"}[i]||"Hoy";else{const b=d.toLocaleDateString(i,{weekday:"short",day:"numeric",month:"short"});y=b.charAt(0).toUpperCase()+b.slice(1)}const f={1:e.dashboard.eventTypeStudyBlock,2:e.dashboard.eventTypeExam,3:e.dashboard.eventTypeImportantTask,4:e.dashboard.eventTypePersonal},p=a.type===2?"exam":a.type===3?"task":a.type===4?"personal":"";o.innerHTML=`
            <div class="space-y-4">
                <div class="event-item ${p} p-4">
                    <div class="mb-2">
                        <span class="inline-flex text-xs px-2 py-1 rounded bg-purple-500/20 text-purple-400 mb-2">${f[a.type]||"Evento"}</span>
                        <h3 class="text-lg font-bold text-white leading-tight">${a.title}</h3>
                    </div>
                    <div class="flex items-center gap-4 text-sm text-gray-400">
                        <div class="flex items-center gap-1">
                            <i data-lucide="calendar" class="w-4 h-4"></i>
                            <span class="font-medium text-purple-300">${y}</span>
                        </div>
                        <div class="flex items-center gap-1">
        ...
        `,D()}}async function Tt(){const e=document.getElementById("dashboard-content");e&&(e.innerHTML='<p class="loading">Cargando datos del dashboard...</p>');try{try{const r=await q(),n=r.language||r.preferences?.language;n&&n!==F()&&(ot(n),ae())}catch{}lt();const o=document.getElementById("pomodoroTagline");if(o){const r=F(),n={es:"Define tu ritmo. Domina tu tiempo.",en:"Set your rhythm. Master your time.",zh:"设定你的节奏，掌控你的时间。",pt:"Defina seu ritmo. Domine seu tempo."};o.textContent=n[r]||n.es}await Promise.all([ut(),mt(),J(),V(),ye(),pe(),St()]),ct(()=>{Promise.all([wt(),Pe(),Et()])}),e&&(e.style.display="none")}catch(o){console.error("Error loading dashboard:",o),e&&(e.innerHTML='<p class="error-state">Error al cargar el dashboard. Por favor, recargá la página.</p>')}}let Me=!1;function De(){if(Me)return;Me=!0,et(),pt(),Tt();const e=document.getElementById("addObjectiveBtn");e&&e.addEventListener("click",ft);const o=document.getElementById("closeObjectiveModalBtn");o&&o.addEventListener("click",ne);const r=document.getElementById("cancelObjectiveBtn");r&&r.addEventListener("click",ne);const n=document.getElementById("objectiveForm");n&&n.addEventListener("submit",ht);const t=document.getElementById("objectiveModal");t&&t.addEventListener("click",a=>{a.target===t&&ne()})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",De,{once:!0}):De();(()=>{let o={...{workMinutes:25,shortBreakMinutes:5,longBreakMinutes:15,cyclesBeforeLongBreak:4}},r=null,n=o.workMinutes*60,t="work",a=!1,s=0,c={totalSessionsThisWeek:0,sessionsToday:0},i=Intl.DateTimeFormat().resolvedOptions().timeZone,d=null,m=null,y=null,f=null,p=null,b=null,h=0,x=null;window.__studyoCaptureFocusState=()=>{try{return{type:"pomodoro",isRunning:a,remainingSeconds:n,currentStage:t,timerStartTime:b,timerStartRemaining:h,settings:{...o},defaultSubjectId:f,completedCycles:s,sessionStart:y?y.toISOString():null,capturedAt:new Date().toISOString()}}catch{return null}};let w=null,E=!1;const O=document.getElementById("pomodoroProgressCircle"),I=document.getElementById("pomodoroTimerDisplay"),g=document.getElementById("pomodoroStageLabel"),k=document.getElementById("pomodorosCount"),T=document.getElementById("pomodoroSetLabel"),A=document.getElementById("pomodoroPlayBtn"),U=document.getElementById("pomodoroSkipBtn"),P=document.getElementById("pomodoroResetBtn"),$=document.getElementById("pomodoroSettingsBtn"),re=document.getElementById("pomodoroTooltip"),_e=document.getElementById("closePomodoroTooltip");let z=null;const N=document.getElementById("pomodoroSettingsModal"),fe=document.getElementById("pomodoroSettingsForm"),Q=document.getElementById("workMinutes"),X=document.getElementById("shortBreakMinutes"),ee=document.getElementById("longBreakMinutes"),te=document.getElementById("cyclesBeforeLongBreak"),G=document.getElementById("pomodoroDefaultSubjectSelect"),he=document.getElementById("pomodoroSettingsCancel"),Ne=552;function He(){if(ue()?.active||!re||localStorage.getItem("studyo_onboarding_done"))return;setTimeout(()=>{localStorage.getItem("studyo_onboarding_done")||re.classList.remove("hidden")},2e3);const l=()=>{re.classList.add("hidden"),localStorage.setItem("studyo_onboarding_done","true")};_e?.addEventListener("click",v=>{v.stopPropagation(),l()}),$?.addEventListener("click",l)}function ie(){const u=ue();if(!u||!u.active){z=null,R();return}if(z=u.step,z==="CREATE_SUBJECT"){W({title:S().onboarding?.stepCreateSubjectTitle||"Paso 1: Creá tu primera asignatura",body:S().onboarding?.stepCreateSubjectBody||"Creá 1 asignatura para activar el tracking real de tu estudio.",primaryText:S().onboarding?.createSubject||"Crear asignatura",primaryHref:"/subjects",lockClose:!0,allowSkip:!0,onSkip:async()=>{try{await Ie()}catch{}R()}});return}if(z==="CREATE_HABIT"){W({title:S().onboarding?.stepCreateHabitTitle||"Paso 2: Creá un hábito",body:S().onboarding?.stepCreateHabitBody||"Ahora creá un hábito y revisá “Hábito clave”.",primaryText:S().onboarding?.goToHabits||"Ir a hábitos",primaryHref:"/habits",lockClose:!0,allowSkip:!0,onSkip:async()=>{try{await Ie()}catch{}R()}});return}if(z==="CONFIG_POMODORO"){W({title:S().onboarding?.stepConfigPomodoroTitle||"Paso 3: Configurá Pomodoro",body:S().onboarding?.stepConfigPomodoroBody||"Definí una materia por defecto para registrar bien tus sesiones.",primaryText:S().dashboard.pomodoroSettings,lockClose:!0,onPrimary:()=>{R(),$?.dispatchEvent(new Event("click"))}});return}if(z==="START_SESSION"){f?W({title:"Paso 4: IniciÃ¡ tu primera sesiÃ³n",body:'Â¡Todo listo! PresionÃ¡ "Empezar ahora" para terminar el onboarding y continuar libremente.',primaryText:"Empezar ahora",lockClose:!0,onPrimary:async()=>{try{await rt(),z="DONE",R();try{ge("Onboarding completado. Ya podes explorar StudyO libremente.")}catch{}}catch(l){console.warn("onboarding completion failed",l)}}}):W({title:"SeleccionÃ¡ una asignatura",body:"Antes de iniciar tu primera sesiÃ³n, elegÃ­ una materia en los ajustes de Pomodoro.",primaryText:"Ir a ajustes",lockClose:!0,onPrimary:()=>{R(),$?.dispatchEvent(new Event("click"))}});return}R()}function Ae(){try{const u=localStorage.getItem("pomodoroSettings");if(u){const l=JSON.parse(u);o={...o,...l},typeof l.defaultSubjectId=="number"&&(f=l.defaultSubjectId)}}catch{}}function ze(){w||(w=new Audio("public/sounds/pomodoroSound.mp3"),w.volume=.8,w.play().then(()=>{w.pause(),w.currentTime=0}).catch(()=>{}))}function Re(){localStorage.setItem("pomodoroSettings",JSON.stringify({...o,defaultSubjectId:f}))}function qe(u){const l=Math.floor(u/60).toString().padStart(2,"0"),v=Math.floor(u%60).toString().padStart(2,"0");return`${l}:${v}`}function K(u,l){return new Intl.DateTimeFormat("en-CA",{timeZone:l,year:"numeric",month:"2-digit",day:"2-digit"}).format(u)}function _(){if(I&&(I.textContent=qe(n)),g){const u=F(),l={es:"Enfoque",en:"Focus",zh:"专注",pt:"Foco"},v={es:"Descanso corto",en:"Short break",zh:"短休息",pt:"Pausa curta"},C={es:"Descanso largo",en:"Long break",zh:"长休息",pt:"Pausa longa"};t==="work"?g.textContent=l[u]||l.es:t==="short_break"?g.textContent=v[u]||v.es:g.textContent=C[u]||C.es}if(O){const u=(t==="work"?o.workMinutes:t==="short_break"?o.shortBreakMinutes:o.longBreakMinutes)*60,l=Math.max(0,Math.min(1,1-n/u)),v=Math.round(Ne*(1-l));O.style.strokeDashoffset=`${v}`}try{const u=F(),l={es:"Pomodoros",en:"Pomodoros",zh:"番茄钟",pt:"Pomodoros"},v={es:"Set: {current} / {total}",en:"Set: {current} / {total}",zh:"组: {current} / {total}",pt:"Conjunto: {current} / {total}"};try{const B=c.sessionsToday||0;if(k&&(k.textContent=`${l[u]||l.es}: ${B}`),T){const H=Math.max(1,o.cyclesBeforeLongBreak);let j=B%H;j===0&&B>0&&(j=H);const M=v[u]||v.es;T.textContent=M.replace("{current}",String(j)).replace("{total}",String(H))}}catch{}}catch{}}document.addEventListener("pomodoro:completed",()=>{try{J().catch(u=>console.warn("refresh stats failed",u)),ye().catch(u=>console.warn("refresh weekly challenge failed",u))}catch{}}),document.addEventListener("pomodoro:sessionsUpdated",async u=>{try{const l=u,v=Number(l.detail?.totalSessionsThisWeek)||0,C=Number(l.detail?.totalSessionsToday)||0;let B=i;try{B=(await q()).timezone||B,i=B}catch{}const H=K(new Date,B),j="pomodoroReset";let M={};try{M=JSON.parse(localStorage.getItem(j)||"{}")}catch{M={}}M.day!==H&&(M={},localStorage.removeItem(j));const de=Number(M.offset||0),Se=Math.max(0,C-de);c.rawTotalSessionsThisWeek=v,c.rawSessionsToday=C,c.totalSessionsThisWeek=v,c.sessionsToday=Se,s=Se,_()}catch{}});function be(){if(w)try{w.currentTime=0,w.play().catch(()=>{})}catch(u){console.warn("Audio not available",u)}}function se(){if(!a||b===null)return;const u=Date.now(),l=Math.floor((u-b)/1e3),v=Math.max(0,h-l);v!==n&&(n=v,_()),n<=0&&ce()}async function ce(){if(be(),document.visibilityState!=="visible"&&Ge(),r&&(clearInterval(r),r=null),t==="work"&&y){const u=new Date;let l=Math.round((u.getTime()-y.getTime())/6e4);l=Math.max(1,l);try{console.log("[finishTimer] Pomodoro finished - creating session",{start:y.toISOString(),end:u.toISOString(),duration:l,subject:f});const v=await Ee(y,u,l,f);console.log("[finishTimer] Pomodoro POST completed",v)}catch(v){console.error("[finishTimer] Error saving pomodoro session",v)}y=null}we(),n=(t==="work"?o.workMinutes:t==="short_break"?o.shortBreakMinutes:o.longBreakMinutes)*60,b=Date.now(),h=n,t==="work"&&!y&&(console.log("[finishTimer] Entering work stage - initializing sessionStart"),y=new Date),r=window.setInterval(()=>{se(),n>0&&(n-=1),n<=0&&ce(),_()},1e3),_()}async function Fe(){if(!a){if((z==="CONFIG_POMODORO"||z==="START_SESSION")&&!f){W({title:"Seleccioná una asignatura",body:"Durante el onboarding no se puede iniciar pomodoro sin materia asignada.",primaryText:"Configurar ahora",lockClose:!0,onPrimary:()=>{R(),$?.dispatchEvent(new Event("click"))}});return}a=!0,le(),ze(),We(),!y&&t==="work"&&(y=new Date),b=Date.now(),h=n,p=u=>{u.preventDefault();const l=F(),v={es:"El Pomodoro se detendrá si abandonas esta página. ¿Estás seguro?",en:"The Pomodoro will stop if you leave this page. Are you sure?",zh:"离开此页面将停止番茄钟，确定继续吗？",pt:"O Pomodoro será interrompido se você sair desta página. Tem certeza?"};return u.returnValue=v[l]||v.es,u.returnValue},window.addEventListener("beforeunload",p),x||(x=()=>{!document.hidden&&a&&se()},document.addEventListener("visibilitychange",x)),r=window.setInterval(()=>{se(),n>0&&(n-=1),n<=0&&ce(),_()},1e3)}}function ve(){r&&(clearInterval(r),r=null),a=!1,b=null,le(),p&&(window.removeEventListener("beforeunload",p),p=null)}function xe(u=null){ve(),u&&(t=u),n=(t==="work"?o.workMinutes:t==="short_break"?o.shortBreakMinutes:o.longBreakMinutes)*60,y=null,b=null,h=0,_()}function Ue(){if(r&&(clearInterval(r),r=null),t==="work"&&y){const u=new Date,l=Math.max(1,Math.round((u.getTime()-y.getTime())/6e4));Ee(y,u,l,f).catch(v=>console.error(v)),y=null}be(),a=!1,le(),we()}function we(){t==="work"?s>0&&s%o.cyclesBeforeLongBreak===0?t="long_break":t="short_break":t="work",n=(t==="work"?o.workMinutes:t==="short_break"?o.shortBreakMinutes:o.longBreakMinutes)*60,_()}function We(){E||(E=!0,"Notification"in window&&Notification.permission==="default"&&Notification.requestPermission())}function Ge(){"Notification"in window&&Notification.permission==="granted"&&new Notification("StudyO",{body:"Pomodoro finalizado.¡Hora de un descanso!",silent:!1})}async function Ee(u,l,v,C){try{const B={start_time:u.toISOString(),end_time:l.toISOString(),duration:v};C!==null&&!isNaN(C)&&(B.subject=Number(C)),console.log("[saveWorkSession] Saving pomodoro:",B);const H=await Le("/pomodoro/",B,!0);console.log("[saveWorkSession] Pomodoro saved successfully",H),console.log("[saveWorkSession] Pomodoro POST sent");try{document.dispatchEvent(new CustomEvent("pomodoro:completed"))}catch{}try{document.dispatchEvent(new CustomEvent("weeklyChallenge:update"))}catch{}await J().catch(j=>console.warn("refresh stats failed",j)),await Pe().catch(j=>console.warn("refresh distribution failed",j));try{_()}catch{}return H}catch(B){throw console.error("Failed to save pomodoro session",B),B}}async function ke(){if(G)try{const u=S(),l=await L("/subjects/");G.innerHTML=`<option value="" data-i18n="dashboard.noPomodoroSubject">${u.dashboard.noPomodoroSubject}</option>`+l.map(v=>`<option value="${v.id}">${v.name}</option>`).join(""),f&&(G.value=f.toString())}catch(u){console.warn("Could not load subjects",u)}}function le(){A&&(A.innerHTML=a?'<i data-lucide="pause" class="w-5 h-5"></i>':'<i data-lucide="play" class="w-5 h-5 ml-1"></i>',D())}document.addEventListener("DOMContentLoaded",async()=>{await nt(),at(()=>ie()),Ae(),He();try{i=(await q()).timezone||i}catch{}d=K(new Date,i),m||(m=window.setInterval(async()=>{try{const l=K(new Date,i);d!==l&&(d=l,localStorage.removeItem("pomodoroReset"),await J(),_())}catch{}},30*1e3)),n=o.workMinutes*60,_(),A&&A.addEventListener("click",async()=>{a?ve():await Fe()}),U&&U.addEventListener("click",()=>Ue()),P&&P.addEventListener("click",async()=>{try{s=0,xe("work"),_();try{const l=await L("/pomodoro/");let v=i;try{v=(await q()).timezone||v,i=v}catch{}const C=K(new Date,v),B=l.filter(M=>{const de=new Date(M.start_time);return K(de,v)===C}).length,H="pomodoroReset",j={day:C,offset:B};localStorage.setItem(H,JSON.stringify(j)),console.log("[reset] Stored reset offset",j),s=0,_(),await J();try{document.dispatchEvent(new CustomEvent("weeklyChallenge:update"))}catch{}}catch(l){console.warn("Could not persist reset offset",l)}}catch(l){console.error("Reset failed",l)}}),$&&N&&$.addEventListener("click",async l=>{l.preventDefault(),!document.body.classList.contains("concentration-mode")&&(Q&&(Q.value=String(o.workMinutes)),X&&(X.value=String(o.shortBreakMinutes)),ee&&(ee.value=String(o.longBreakMinutes)),te&&(te.value=String(o.cyclesBeforeLongBreak)),await ke(),N.style.display="flex",D())}),he&&N&&he.addEventListener("click",()=>{N.style.display="none"});const u=document.getElementById("pomodoroSettingsCloseBtn");u&&N&&u.addEventListener("click",()=>{N.style.display="none"}),N&&N.addEventListener("click",l=>{l.target===N&&(N.style.display="none")}),fe&&N&&fe.addEventListener("submit",l=>{l.preventDefault();const v=Number(Q&&Q.value||o.workMinutes),C=Number(X&&X.value||o.shortBreakMinutes),B=Number(ee&&ee.value||o.longBreakMinutes),H=Number(te&&te.value||o.cyclesBeforeLongBreak);o.workMinutes=Math.max(1,v),o.shortBreakMinutes=Math.max(1,C),o.longBreakMinutes=Math.max(1,B),o.cyclesBeforeLongBreak=Math.max(1,H);const j=G?G.value:"";f=j?Number(j):null,z==="CONFIG_POMODORO"&&f&&it("START_SESSION").then(()=>{const M=ue();M&&(M.step="START_SESSION"),z="START_SESSION",ie()}).catch(()=>{}),Re(),a||xe("work"),N.style.display="none",_()}),ke(),ie()})})();
