                    ...
                    const renderPagination = (currentPage) => {
                        ...
                        ${pageNumbers.map((p) => `
                            <button type="button" data-page="${p}"
                                class="w-9 h-9 sm:w-10 sm:h-10 rounded-full border text-sm font-semibold transition-all ${p === currentPage
                            ? 'border-purple-400 bg-purple-600 text-white shadow-[0_0_12px_rgba(168,85,247,0.35)]'
                            : 'border-gray-700 bg-dark-input text-gray-300 hover:border-purple-500/60'}">${p}</button>
                        `).join('')}
