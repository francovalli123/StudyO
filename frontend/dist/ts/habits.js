 ...
 */
function getUserToday(userTimezone) {
...
function initHabitsGridDelegation() {
    if (habitsGridDelegationBound)
        return;
    const grid = document.getElementById("habitsGrid");
    if (!grid)
        return;
    grid.addEventListener('click', (event) => __awaiter(this, void 0, void 0, function* () {
        const target = event.target;
        if (!target)
            return;
        const completeBtn = target.closest('.habit-complete-btn');
        if (completeBtn) {
        ...
        const editBtn = target.closest('.edit-habit-btn');
        if (editBtn) {
